...
SHOWING_PHASE_DURATION = 3000  # 30 seconds in milliseconds
...
def display_matching_game(target_icon, correct_icon, random_icons, selected_index, incorrect_choices, animation_state, positions, animation_progress, decision_message, num_choices):
    ...
    # Always show correct_icon if forcing it.
    if force_correct:
...
# 1. Generate 4 new pairs (from the current images)
# Load daily prompts if available
...
if not daily_new_pairs:
    # Generate new pairs if daily file not found or empty.
    ...
    # Save the daily prompts (filenames only)
    daily_data = {
        "day": today_str,
...
while running:
    ...
    if game_state == "showing_pairs":
        ...
        # Transition to matching phase if time is up
        if elapsed >= SHOWING_PHASE_DURATION:
            game_state = "matching_game"
            showing_phase_start = None
            # Initialize matching phase with first pair from pairs
    ...
    # --- Matching Phase ---
        ...
        if animation_state in ["correct", "incorrect"]:
            ...
            if current_time - decision_start_time > DECISION_DISPLAY_DURATION:
                ...
                animation_progress["distance"] = 0
                force_correct = False
                # Update to next pair if available
        ...
        if event.type == pygame.KEYDOWN:
            if game_state == "showing_pairs":
                if event.key == pygame.K_LEFT:
                    ...
                    # Optionally, SPACE also cycles forward.
                    pair_index = (pair_index + 1) % len(pairs)
                ...
                if event.key == pygame.K_RIGHT:
                    ...
                    if random_icons[selected_box_index]["path"] == correct_icon["path"]:
                        ...
                        force_correct = True
                        # Instead of immediately updating, store next pair.
                        if len(matched_pairs) < len(pairs):
...
# Save game_results in Score with the current date as filename
folder = "VPAT/Score"
if not os.path.exists(folder):
    os.makedirs(folder)
current_date = datetime.now().strftime("%Y-%m-%d")
