...
def display_matching_game(target_icon, correct_icon, random_icons, selected_index, incorrect_choices, animation_state, positions, animation_progress, decision_message, num_choices):
    ...
    if animation_state == "moving_apart":
        ...
        # Draw rectangles around the clashed squares
    ...
    # Always show correct_icon if we're forcing it.
    if force_correct:
    ...
    # (Grid layout code remains unchanged below)
    if num_choices == 5:
    ...
    grid_start_y = SCREEN_HEIGHT - total_grid_height - 50  # 50 pixel margin from bottom
...
daily_prompts_file = os.path.join("VPAT", f"new_prompts.json")

# Function to convert saved daily prompts to game pairs format.
...
# Try to load today's new pairs from the daily file.
daily_new_pairs = load_new_pairs_from_daily()

# If the file doesn't exist or is empty, generate new pairs as before.
if not daily_new_pairs:
    # [Your previous new_pairs generation code goes here, including filtering used files, etc.]
    # For example:
    ...
    # Save the daily prompts (filenames only) as before:
    daily_data = {
        "day": today_str,
    ...
    # Append these new pairs to the master used prompts file
...
# Now use daily_new_pairs as your new pairs.
new_pairs = daily_new_pairs

# Then later, when building your overall pairs list:
...
# Now pairs will have up to 10 pairs. (If previous data isn’t available, you'll only have new pairs.)
...
game_results = []
game_state = "showing_pairs"  # states: 'showing_pairs', 'matching_game'
    ...
    if game_state == "showing_pairs":
        ...
        if animation_state == "correct" or animation_state == "incorrect":
            ...
            if pygame.time.get_ticks() - decision_start_time > DECISION_DISPLAY_DURATION:
                ...
                animation_progress["distance"] = 0
                force_correct = False 
                 # Now update to the next pair (if available)
                if next_pair is not None:
                    target_icon = next_pair[0]
                    correct_icon = next_pair[1]
                    # Reinitialize random_icons for the new correct_icon
                    available_icons = [
                        {"surface": icon, "path": os.path.join(icon_path, os.listdir(icon_path)[icons.index(icon)])}
                        for icon in icons
                        if icon != correct_icon["surface"]
            ...
            if game_state == "showing_pairs" and event.key == pygame.K_SPACE:
                ...
                if event.key == pygame.K_RIGHT:
                    ...
                    if random_icons[selected_box_index]["path"] == correct_icon["path"]:
                        ...
                        force_correct = True
                         # Instead of updating target_icon and correct_icon immediately, store next pair
                        if len(matched_pairs) < len(pairs):
