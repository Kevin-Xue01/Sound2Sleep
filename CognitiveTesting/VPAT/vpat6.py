...
class SleepStudyApp(QWidget):
    ...
    # ---------------------------
    def create_medal_label(self, rank):
        """Returns a QLabel styled as a medal for ranks 1, 2, or 3."""
        medal_label = QLabel(str(rank))
        medal_label.setAlignment(Qt.AlignCenter)
        medal_label.setFixedSize(30, 30)
        if rank == 1:
            medal_label.setStyleSheet("background-color: gold; color: black; border-radius: 15px; font-weight: bold;")
        elif rank == 2:
            medal_label.setStyleSheet("background-color: silver; color: black; border-radius: 15px; font-weight: bold;")
        elif rank == 3:
            medal_label.setStyleSheet("background-color: #cd7f32; color: black; border-radius: 15px; font-weight: bold;")
        else:
            medal_label.setStyleSheet("color: white;")
        return medal_label
    ...
    def update_scoreboard(self):
        ...
        grid_ninja.setVerticalSpacing(5)
        ...
        user_score_ninja = self.get_score(score1_file)
        ...
        for row, (name, score) in enumerate(ninja_entries, start=1):
            ...
            # If rank 1, 2, or 3, display medal instead of plain number.
            if current_rank in (1, 2, 3):
                medal = self.create_medal_label(current_rank)
                grid_ninja.addWidget(medal, row, 0)
            else:
                rank_label = QLabel(str(current_rank))
        ...
        grid_memorize.setVerticalSpacing(5)
        for col, text in enumerate(headers):
            label = QLabel(text)
            label.setFont(header_font)
            label.setStyleSheet("color: white;")
        ...
        user_score_memorize = self.get_score(score2_file)
        ...
        for row, (name, score) in enumerate(memorize_entries, start=1):
            ...
            if current_rank in (1, 2, 3):
                medal = self.create_medal_label(current_rank)
                grid_memorize.addWidget(medal, row, 0)
            else:
                rank_label = QLabel(str(current_rank))
                rank_label.setAlignment(Qt.AlignCenter)
        ...
        current_rankings = {
            "ninja_swipe": ninja_rankings,
            "can_you_memorize": memorize_rankings
        }
