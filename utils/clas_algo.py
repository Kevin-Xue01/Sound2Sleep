    ...
    def update(self, processor_elapsed_time, data):
        ...
        if isnan(self.second_stim_start):
            ...
            if ((self.last_stim + self.config.backoff_time) > (self.processor_elapsed_time + self.config.stim1_prediction_limit_sec)):
                return CLASAlgoResult(CLASAlgoResultType.BACKOFF, 0, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)

            ### check amplitude criteria ###
            if (mean_amp < self.config.amp_threshold) or (mean_amp > self.config.amp_limit):
                return CLASAlgoResult(CLASAlgoResultType.AMPLITUDE, 0, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)

            ### check quadrature ###
            if (quadrature is not None) and (quadrature < self.config.quadrature_thresh):
                return CLASAlgoResult(CLASAlgoResultType.QUADRATURE, 0, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)


            if (mean_hl_ratio > self.config.hl_ratio_buffer_threshold) or (current_hl_ratio > self.config.hl_ratio_latest_threshold):
                return CLASAlgoResult(CLASAlgoResultType.HL_RATIO, 0, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)
        ...
        if self.processor_elapsed_time < self.second_stim_start:
            return CLASAlgoResult(CLASAlgoResultType.BACKOFF2, 0, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)
        ...
        if isnan(self.second_stim_start):
            if delta_t > self.config.stim1_prediction_limit_sec:
                return CLASAlgoResult(CLASAlgoResultType.FUTURE, delta_t, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)
            ...
            return CLASAlgoResult(CLASAlgoResultType.STIM, delta_t, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)

        else:
            if delta_t > self.config.stim2_prediction_limit_sec:
                return CLASAlgoResult(CLASAlgoResultType.FUTURE2, delta_t, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)

            self.second_stim_start = nan
            self.second_stim_end = nan

            return CLASAlgoResult(CLASAlgoResultType.STIM2, delta_t, phase, freq, current_amp, mean_amp, current_hl_ratio, mean_hl_ratio, quadrature)
